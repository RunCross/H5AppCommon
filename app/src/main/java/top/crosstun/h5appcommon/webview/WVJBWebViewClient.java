...
public class WVJBWebViewClient extends WebViewClient {
    ...
    public boolean shouldOverrideUrlLoading(WebView view, String url) {
        ...
        if (url.startsWith(kCustomProtocolScheme)) {
            if (url.indexOf(kQueueHasMessage) > 0) {
                flushMessageQueue();
            } else if (url.indexOf(BRIDGE_LOADED) > 0) {
                registerBridge();
            }
            return true;
        } else if (url.startsWith("https") && url.indexOf(BRIDGE_LOADED) > 0) {
            registerBridge();
            return true;
        }
    ...
    public void onLoadResource(WebView view, String url) {
        Log.e("top.crossrun.onload", url);
        if (url.indexOf(BRIDGE_LOADED) > 0 && (url.startsWith(kCustomProtocolScheme) || url.startsWith("https"))) {
            registerBridge();
            return;
        }
